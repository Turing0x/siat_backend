...
async function updateExercise(req: Request, res: Response) {
  ...
  try {
    ...
    (await UserModel.find({ type: "student" })).forEach(
      async ({ _id, group, pending_exercices }) => {
        if (updated.destine === "0") {
          if (!pending_exercices.includes(id)) {
            await UserModel.findByIdAndUpdate(_id, {
              $push: { pending_exercices: id },
            }).then();
          }
        } else if (group === updated.destine) {
          await UserModel.findByIdAndUpdate(_id, {
            $push: { pending_exercices: id },
          }).then();
        } else {
          await UserModel.findByIdAndUpdate(_id, {
            $pull: { pending_exercices: id },
          }).then();
        }
      }
    );
...
async function deleteExerciseById(req: Request, res: Response) {
  ...
  try {
    ...
    [exercises.exercise_files, exercises.solution].forEach(
      async (file, index) => {
